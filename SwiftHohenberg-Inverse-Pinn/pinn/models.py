...
class RBF_PINNs(tf.keras.layers.Layer):
    def __init__(
        ...
        min_lr,
        max_lr,
        u,  # This 'u' is the u_n numpy array from main.py
        dx,
        tol,
        threshold_ep,
        batchsize,
        ...
        # These variables are not used since normalization is done in main.py
        # but they were in your original file.
        ...
        # This 'u' parameter is the numpy array u_tp
    ...
    # -----------------------------------------------------------------
    # --- !!! DANGER: THIS FUNCTION CAUSES THE LAPTOP FREEZE !!! ---
    # It runs the 4th-order gradient on ALL 262,144 points at once.
    # -----------------------------------------------------------------
        ...
        while self.iterations > -1:
            if self.iterations < self.threshold:
                ...
                if first_pass or self.iterations % update_freq == 0:
                    self.update_dynamic_weights() # <--- FREEZE HAPPENS HERE
                    first_pass = False
